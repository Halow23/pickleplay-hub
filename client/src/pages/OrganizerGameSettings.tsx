...
export default function OrganizerGameSettings() {
  ...
  const update = trpc.organizer.updateGame.useMutation({ onSuccess: () => { toast.success("Game capacity and RSVP deadline saved."); utils.organizer.games.invalidate(); }, onError: error => toast.error(error.message) });
  const cancel = trpc.organizer.cancelGame.useMutation({ onSuccess: () => { toast.success("Game cancelled and participants notified."); setCancellationReason(""); utils.organizer.games.invalidate(); }, onError: error => toast.error(error.message) });
