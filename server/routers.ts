...
import { addVenueSource, addVenueStaff, listVenueReviews, reviewVenueClaim, reviewVenueCorrection, setVenueCoordinates, setVenueVerificationState, submitVenueClaim, submitVenueCorrection } from "./venueService";
import { countUnreadMessages, listConversations, listDirectMessages, sendDirectMessage } from "./directMessageService";
...
export const appRouter = router({
  ...
  community: router({
    ...
    conversations: protectedProcedure.query(async ({ ctx }) => { try { return await listConversations(ctx.user.id); } catch (error) { return communityError(error); } }),
    directThread: protectedProcedure.input(z.object({ counterpartId: z.number().int().positive() })).query(async ({ ctx, input }) => { try { return await listDirectMessages(ctx.user.id, input.counterpartId); } catch (error) { return communityError(error); } }),
    sendDirectMessage: protectedProcedure.input(z.object({ recipientId: z.number().int().positive(), body: z.string().trim().min(1).max(2000) })).mutation(async ({ ctx, input }) => { try { return await sendDirectMessage(ctx.user.id, input.recipientId, input.body); } catch (error) { return communityError(error); } }),
    unreadMessageCount: protectedProcedure.query(async ({ ctx }) => { try { return { count: await countUnreadMessages(ctx.user.id) }; } catch (error) { return communityError(error); } }),
  ...
  admin: router({
    ...
    setVenueCoordinates: protectedProcedure.input(z.object({ venueId: z.number().int().positive(), lat: z.number().min(-90).max(90).nullable(), lng: z.number().min(-180).max(180).nullable() })).mutation(async ({ ctx, input }) => { try { return await setVenueCoordinates(ctx.user, input.venueId, input.lat, input.lng); } catch (error) { return communityError(error); } }),
