...
function EmbeddedProfileNotificationControls({ preferences, isLoading, isSaving, error, onChange }: { preferences?: { inAppEnabled: boolean; emailEnabled: boolean; gameUpdatesEnabled: boolean; waitlistUpdatesEnabled: boolean }; isLoading: boolean; isSaving: boolean; error?: string; onChange: (change: Partial<{ inAppEnabled: boolean; emailEnabled: boolean; gameUpdatesEnabled: boolean; waitlistUpdatesEnabled: boolean }>) => void }) {
  const rows = preferences ? [{ key: "inAppEnabled", label: "In-app updates", value: preferences.inAppEnabled, detail: "Show important PicklePlay coordination updates here." }, { key: "emailEnabled", label: "Email updates", value: preferences.emailEnabled, detail: "Also send these updates to your account email when the server has email configured." }, { key: "gameUpdatesEnabled", label: "Organizer updates", value: preferences.gameUpdatesEnabled, detail: "Receive notes for games you joined or are waiting for." }, { key: "waitlistUpdatesEnabled", label: "Waitlist changes", value: preferences.waitlistUpdatesEnabled, detail: "Receive confirmations and promotion updates." }] as const : [];
  return <section className="mt-6 rounded-2xl border border-[#dfe1d5] bg-white p-4" aria-label="Notification preferences"><div className="flex items-start gap-3"><Bell className="mt-0.5 h-5 w-5 text-[#39705d]" /><div><p className="text-[10px] font-extrabold uppercase tracking-[.11em] text-[#5f6f66]">Notification preferences</p><p className="mt-1 text-sm leading-6 text-[#68756d]">Choose which coordination updates appear in PicklePlay, in-app or by email.</p></div></div>{isLoading ? <p className="mt-4 text-sm text-[#68756d]">Loading your preferences…</p> : error ? <p role="alert" className="mt-4 rounded-xl bg-[#fff0e9] p-3 text-sm text-[#98513d]">Could not load preferences. {error}</p> : <div className="mt-4 space-y-2">{rows.map(row => <div key={row.key} className="flex items-center justify-between gap-4 rounded-xl bg-[#f1f4ec] p-3"><div><p className="text-sm font-bold">{row.label}</p><p className="mt-0.5 text-xs leading-5 text-[#68756d]">{row.detail}</p></div><Switch checked={row.value} onCheckedChange={checked => onChange({ [row.key]: checked } as Partial<{ inAppEnabled: boolean; gameUpdatesEnabled: boolean; waitlistUpdatesEnabled: boolean }>)} disabled={isSaving} aria-label={row.label} /></div>)}</div>}{isSaving && <p className="mt-3 text-xs font-bold text-[#39705d]" aria-live="polite">Saving your preference…</p>}{!isLoading && <a href="/settings/notifications" className="mt-4 inline-flex text-xs font-bold text-[#39705d] underline underline-offset-4">Open full notification settings</a>}</section>;
}
...
function CapacityMeter({ confirmed, capacity }: { confirmed: number; capacity: number }) {
  const percentage = capacity > 0 ? Math.min(100, Math.round((confirmed / capacity) * 100)) : 0;
  return (
    <div className="mt-4">
      ...
      <div
        role="progressbar"
        aria-label="Game capacity filled"
        aria-valuemin={0}
        aria-valuemax={capacity}
        aria-valuenow={confirmed}
        className="h-1.5 overflow-hidden rounded-full bg-[#e6e6dc]"
      >
...
export default function Home() {
  ...
  const createInviteMutation = trpc.organizer.createGroupInvite.useMutation({
    onSuccess: result => { const link = `${window.location.origin}/?view=groups&invite=${encodeURIComponent(result.token)}`; setInviteToken(result.token); navigator.clipboard?.writeText(link); toast.success("Shareable invite link copied. It expires in 7 days."); },
    onError: error => toast.error(error.message),
  });
  const acceptInviteMutation = trpc.community.acceptGroupInvite.useMutation({
    onSuccess: () => { setInviteToken(""); window.history.replaceState({}, "", `${window.location.pathname}?view=groups`); toast.success("You joined the invited group."); utils.community.dashboard.invalidate(); },
    onError: error => toast.error(error.message),
  });
  ...
  const handleReport = (subjectType: "game" | "group", subjectId: number) => {
    if (!isAuthenticated) return askForSignIn();
    setReportDialog({ subjectType, subjectId, reason: "" });
  };

  const handleBlockHost = (hostId: number) => {
    if (!isAuthenticated) return askForSignIn();
    setBlockDialog({ hostId, reason: "" });
  };
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#f5f3eb] text-[#173d35]">
          ...
          <>
            <section className="grid gap-5 xl:grid-cols-[minmax(0,1.7fr)_minmax(310px,.75fr)]">
              ...
              <aside className="rounded-[32px] border border-[#dfe1d5] bg-[#fffef9] p-6 shadow-[0_14px_32px_rgba(61,80,66,.06)]">
                <div className="flex items-center justify-between"><p className="text-[11px] font-extrabold uppercase tracking-[.13em] text-[#55685e]">Start here</p><span className="rounded-full bg-[#eaf2e8] px-2.5 py-1 text-[10px] font-bold text-[#39705d]">Community first</span></div>
            ...
            <section className="mt-9"><div className="mb-4 flex flex-wrap items-end justify-between gap-3"><div><p className="text-[11px] font-extrabold uppercase tracking-[.14em] text-[#55685e]">Near you</p><h2 className="mt-1 font-[Fraunces] text-[1.75rem] font-semibold tracking-[-.04em]">Good places to begin</h2></div><button onClick={() => setActiveView("play")} className="flex items-center gap-1 text-sm font-bold text-[#2f6e5c] hover:text-[#173d35]">See all games <ChevronRight className="h-4 w-4" /></button></div>
              <div className="grid gap-4 lg:grid-cols-3">{dashboardQuery.isLoading ? [1, 2, 3].map(item => <div key={item} className="h-[208px] animate-pulse rounded-[25px] bg-[#e8e9df]" />) : dashboard?.venues.length ? dashboard.venues.slice(0, 3).map((venue, index) => <article key={venue.id} className="group relative overflow-hidden rounded-[25px] border border-[#dde0d5] bg-[#fffef9] p-5 transition duration-200 hover:-translate-y-1 hover:shadow-[0_18px_34px_rgba(52,79,67,.11)]"><div className={`absolute right-0 top-0 h-20 w-20 rounded-bl-[60px] ${index === 0 ? "bg-[#e6bb62]" : index === 1 ? "bg-[#8fb59e]" : "bg-[#d88a66]"} opacity-80`} /><div className="relative flex h-10 w-10 items-center justify-center rounded-2xl bg-[#f1f4ec] text-[#306b58]"><MapPin className="h-5 w-5" /></div><h3 className="relative mt-5 font-[Fraunces] text-xl font-semibold tracking-[-.03em]">{venue.name}</h3><p className="mt-1 text-sm text-[#68766e]">{venue.neighborhood} · {venue.courtCount} courts</p><div className="mt-5 flex flex-wrap gap-2">{venue.indoor && <span className="rounded-full bg-[#e8f0e7] px-2.5 py-1 text-[10px] font-bold uppercase tracking-[.08em] text-[#39705d]">Indoor</span>}<span className="rounded-full bg-[#f4f1e8] px-2.5 py-1 text-[10px] font-bold uppercase tracking-[.08em] text-[#7a6b52]">{venue.lighting ? "Lit courts" : "Daylight play"}</span></div></article>) : <div className="lg:col-span-3"><EmptyState title="No venues yet" body="Real venue listings will appear once a community member adds a court." /></div>}</div>
            </section>

            <section className="mt-10 grid gap-5 lg:grid-cols-[1.42fr_.58fr]">
              <div className="rounded-[28px] border border-[#dfe1d5] bg-[#fffef9] p-5 sm:p-6"><div className="flex items-center justify-between gap-3"><div><p className="text-[11px] font-extrabold uppercase tracking-[.14em] text-[#55685e]">From the community</p><h2 className="mt-1 font-[Fraunces] text-2xl font-semibold tracking-[-.04em]">Hosts set the tone</h2></div><MessagesSquare className="h-5 w-5 text-[#d17954]" /></div><div className="mt-5 space-y-3">{dashboard?.games.length ? dashboard.games.slice(0, 2).map(game => <div key={game.id} className="rounded-2xl bg-[#f5f6ef] p-4"><div className="flex items-start gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#e4b54a] text-[10px] font-bold">{getInitials(game.organizerName)}</div><div><div className="flex flex-wrap items-center gap-x-2 gap-y-1"><p className="text-sm font-bold">{game.organizerName}</p><span className="text-xs text-[#778278]">in {game.groupName || "Local Play"}</span></div><p className="mt-1.5 text-sm leading-6 text-[#57665d]"><strong className="font-bold text-[#254e43]">{game.postHeadline}</strong> — {game.postBody}</p></div></div></div>) : <EmptyState title="No community notes yet" body="Organizer notes will appear when real games are posted." />}</div></div>
      ...
      <Dialog open={reportDialog !== null} onOpenChange={open => { if (!open) setReportDialog(null); }}><DialogContent className="rounded-[28px] border-[#dfe1d5] bg-[#fffef9] sm:max-w-[480px]"><DialogHeader><DialogTitle className="font-[Fraunces] text-2xl font-semibold tracking-[-.03em]">Report this {reportDialog?.subjectType === "game" ? "game" : "group"}</DialogTitle></DialogHeader><p className="-mt-2 text-sm leading-6 text-[#6b786f]">Tell moderators what happened. Reports are reviewed by the community safety team, and you'll stay anonymous to the reported organizer.</p><form onSubmit={event => { event.preventDefault(); if (!reportDialog) return; reportMutation.mutate({ subjectType: reportDialog.subjectType, subjectId: reportDialog.subjectId, reason: reportDialog.reason }); setReportDialog(null); }} className="mt-3 grid gap-4"><div className="grid gap-2"><Label htmlFor="reportReason">What happened?</Label><Textarea id="reportReason" value={reportDialog?.reason ?? ""} onChange={event => setReportDialog(prev => prev ? { ...prev, reason: event.target.value } : prev)} placeholder="Describe the concern so moderators can act on it." required minLength={3} className="min-h-24" /></div><div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={() => setReportDialog(null)} className="rounded-full">Cancel</Button><Button type="submit" disabled={reportMutation.isPending} className="rounded-full bg-[#19473e] text-white hover:bg-[#123b33]">Send report</Button></div></form></DialogContent></Dialog>
      <Dialog open={blockDialog !== null} onOpenChange={open => { if (!open) setBlockDialog(null); }}><DialogContent className="rounded-[28px] border-[#dfe1d5] bg-[#fffef9] sm:max-w-[480px]"><DialogHeader><DialogTitle className="font-[Fraunces] text-2xl font-semibold tracking-[-.03em]">Hide this host</DialogTitle></DialogHeader><p className="-mt-2 text-sm leading-6 text-[#6b786f]">Blocking hides this host's games from your discovery feed. The host is not notified, and you can undo this at any time.</p><form onSubmit={event => { event.preventDefault(); if (!blockDialog) return; blockUserMutation.mutate({ blockedUserId: blockDialog.hostId, reason: blockDialog.reason }); setBlockDialog(null); }} className="mt-3 grid gap-4"><div className="grid gap-2"><Label htmlFor="blockReason">Why are you hiding them? (optional)</Label><Textarea id="blockReason" value={blockDialog?.reason ?? ""} onChange={event => setBlockDialog(prev => prev ? { ...prev, reason: event.target.value } : prev)} placeholder="Helps us spot repeat problems — never shown to the host." className="min-h-24" /></div><div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={() => setBlockDialog(null)} className="rounded-full">Cancel</Button><Button type="submit" disabled={blockUserMutation.isPending} className="rounded-full bg-[#19473e] text-white hover:bg-[#123b33]">Hide host</Button></div></form></DialogContent></Dialog>
    </div>
  );
}
